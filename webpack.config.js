...
.concat(['aurelia-task-queue','aurelia-validation','awesome-phonenumber','aurelia-i18n'])
.concat(['aurelia-resources','aurelia-deco'])
...
module.exports = ({ environment, production } = {}, {extractCss, analyze, tests, hmr, port, host } = {}) => ({
  resolve: {
    extensions: ['.ts', '.js'],
...
// list resources for plugins in SRC mode
  ...
  { from: './images/**/*', to: './' },
    ...
    ...when(!tests, new CopyWebpackPlugin([
      { from: 'node_modules/aurelia-resources/static', to: outDir }])), // copy static files from aurelia-resources plugin
